...
// Helper to call OpenRouter (existing logic moved for reuse)
async function getChatModelResponse(messages, model, referer) {
...
router.post('/chat', chatLimiter, async (req, res) => {
    try {
        ...
        // Keep conversation window (max 8 messages for speed)
...
/**
 * POST /api/mind71/chat/stream
 * SSE version for streaming chunks immediately to the browser
 */
router.post('/chat/stream', chatLimiter, async (req, res) => {
    try {
        ...
        if (!process.env.OPENROUTER_API_KEY) {
            return res.status(500).json({ success: false, message: "OPENROUTER_API_KEY not configured" });
        }
        ...
        // Send confirmation of conversationId
        res.write(`data: ${JSON.stringify({ conversationId: currentConvId })}\n\n`);
        ...
        const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
            ...
            headers: {
                ...
                "Content-Type": "application/json",
                "HTTP-Referer": referer,
                "X-Title": "YallaStarter Mind71 Streaming"
            },
            body: JSON.stringify({
                model: primaryModel,
                messages: chat.messages.map(m => ({ role: m.role, content: m.content })),
        ...
        if (!response.ok) {
            ...
            res.write(`data: ${JSON.stringify({ error: "Stream start failed", details: errText })}\n\n`);
            return res.end();
        }
